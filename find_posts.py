...
def get_new_followers(server, user_id, max, known_followers, access_token):
    ...
    followers = get_paginated_mastodon(f"https://{server}/api/v1/accounts/{user_id}/followers", max, headers={
        "Authorization": f"Bearer {access_token}",
    })
...
def get_new_followings(server, user_id, max, known_followings, access_token):
    """Get any new followings for the specified user, up to the max number provided"""
    following = get_paginated_mastodon(f"https://{server}/api/v1/accounts/{user_id}/following", max, headers={
        "Authorization": f"Bearer {access_token}",
    })
    ...
    try:
        ...
        for token in arguments.access_token:
            ...
            if arguments.max_followings > 0:
                ...
                followings = get_new_followings(arguments.server, user_id, arguments.max_followings, all_known_users, token)
            ...
            if arguments.max_followers > 0:
                ...
                user_id = get_user_id(arguments.server, arguments.user, token)
                followers = get_new_followers(arguments.server, user_id, arguments.max_followers, all_known_users, token)
